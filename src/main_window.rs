...
// ノート番号をノート名に変換するフォーマッタを取得
fn note_formatter(note_type: &DisplayNoteType) -> fn(f32) -> String {
    match note_type {
        DisplayNoteType::NoteNameMiddleC4 => |note| {
            if note < 0.0 {
                format!("")
            } else {
                ...
                let number = (qnote / 12) as i8 - 1;
                format!("{}{}", name, number)
            }
        },
        DisplayNoteType::NoteNumber => |note| {
            if note < 0.0 {
                format!("")
            } else {
                format!("{:6.2}", note)
...
impl MainWindow {
    pub fn new(
        ...
        Self {
            ...
            pitch_indicator: [Indicator::new(
                ...
                0.0,
                127.0,
                note_formatter(&DisplayNoteType::NoteNumber),
            ); 8],
    ...
    pub fn set_noteindicator_formatter(&mut self, note_type: &DisplayNoteType) {
        for ch in 0..8 {
            self.pitch_indicator[ch].formatter = note_formatter(note_type);
        }
    }
}
